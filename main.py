...
def get_branch_root(branch_key):
    """Возвращает название искры без суффикса '_свет'/'_тьма'."""
...
BRANCH_DISPLAY_NAMES = {
    "плоть_свет": "🌿 Плоть: Путь Целителя",
    "плоть_тьма": "⚔️ Плоть: Путь Разрушителя",
    "материя_свет": "🔨 Материя: Путь Творца",
    "материя_тьма": "⛓️ Материя: Путь Воина Материи",
    "время_свет": "⏳ Время: Путь Хроникёра",
    "время_тьма": "⌛ Время: Путь Разрушителя Времени",
    "сны_свет": "💭 Сны: Путь Мечтателя",
    "сны_тьма": "🌑 Сны: Путь Кошмара",
    "оковы_свет": "🛡️ Оковы: Путь Стража",
    "оковы_тьма": "🔗 Оковы: Путь Тюремщика",
    "тень_свет": "✨ Тень: Путь Целостного",
    "тень_тьма": "🌘 Тень: Путь Поглощённого",
    "свет_свет": "☀️ Свет: Путь Провидца",
    "свет_тьма": "🕶️ Свет: Путь Ослепительного",
    "пространство_свет": "🌀 Пространство: Путь Мандра",
    "пространство_тьма": "🌪️ Пространство: Путь Исказителя",
    "душа_свет": "👼 Душа: Путь Пастыря",
    "душа_тьма": "💀 Душа: Путь Некроманта",
    "хаос_свет": "🎲 Хаос: Путь Игрока",
    "хаос_тьма": "🔥 Хаос: Путь Отчаянного",
    "звук_свет": "🎵 Звук: Путь Гармониста",
    "звук_тьма": "🔇 Звук: Путь Какофониста",
    "разум_свет": "🧠 Разум: Путь Мудреца",
    "разум_тьма": "👁️ Разум: Путь Властелина",
    "равновесие_свет": "⚖️ Равновесие: Путь Миротворца",
    "равновесие_тьма": "🔮 Равновесие: Путь Вершителя",
}
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not unlocked:
        await update.message.reply_text(
            "🌟 Ты пока не разблокировал ни одной ветви магии.\n"
            "Иди, искатель, совершай подвиги и возвращайся с новыми силами!"
        )
        return
    ...
    text = "✨ **Твои разблокированные ветви магии:** ✨\n\n" + "\n".join(lines)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if branch_key:
        ...
        # Получаем множество корней уже полученных ветвей
        existing_roots = {get_branch_root(b) for b in unlocked}

        if branch_root in existing_roots:
            await update.message.reply_text(
                "❌ Ты уже сделал выбор для этой искры. Нельзя идти двумя путями одновременно.\n"
                "Дорога требовала одного решения, и ты его принял."
            )
        elif branch_key in unlocked:
            await update.message.reply_text("⚠️ Ты уже получил эту ветвь магии. Иди дальше, искатель!")
        else:
            ...
            await update.message.reply_text(
                f"🎉 Поздравляю! Ты выполнил условие и разблокировал новую ветвь магии.\n\n{description}"
            )
